...
@section('content')
<h1>{{ trans('admin/manual.manage_blog') }}</h1>

<p>{!! link_to_route('admin.manuals.create', trans('admin/manual.add_new') , null, array('class' => 'btn btn-success')) !!}</p>
<div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">{{ trans('admin/manual.list') }}</div>
        </div>
        <div class="portlet-body">
            {!! Form::open(['method'=>'GET','route' => 'admin.manuals.index','class'=>'navbar-form navbar-left','role'=>'search'])  !!}
        ...
        <input type="text" class="form-control" name="title" placeholder="title"> <span class="input-group-btn">
            ...
            <!--table class="table table-striped table-hover table-responsive datatable  table_layout_fixed manuals_table" id="datatable-manuals" -->
                 <table class="table table-striped table-hover table-responsive   table_layout_fixed manuals_table" id="datatable-manuals">
                            <thead>
                                <tr>  <th>Id</th>
                                    <th>Title</th>
                            ...
                            <tbody>
                                ...
                                <tr> <td>{{ $row->id }}</td>
                                    ...
                                    <td>
                                        {!! link_to_route('admin.manuals.edit', trans('admin/manual.edit'), array($row->id), array('class' => 'btn btn-xs btn-info')) !!} &nbsp;
                                        {{ Html::link(url('/') . "/" . $filepath . "/" .$row->filename  , 'View Pdf',['target'=>'_blank' ,'class' => 'btn btn-xs btn-info'])}} &nbsp;
